...
Mobius Harness 只有 `lite` 和 `full` 两种模式。`lite` 用最终响应承载紧凑阶段状态；`full` 使用 `.delivery/runs/<run-id>/` 持久化 artifact。持久化 artifact 必须包含 `Mode` header 和 `Delegation Ledger`，以便每个阶段记录不同的 subagent role、候选 specialist skill、触发条件、决策、证据和 delegated skill 返回给 Mobius Harness 的结果。Requirements artifact 必须包含 `Instruction Evidence` 和 `Minimum Skill Dependencies`，记录已读取的仓库/路径指令、优先级处理、`mobius-harness`、`local-repo-development`、`superpowers:brainstorming`、`superpowers:writing-plans` 的使用条件、依赖分类、证据和 fallback。Plan artifact 还必须包含 `First-Principles Design Check` 和 `Validation Prerequisites`，记录核心目标、必须保持的 invariant、所属机制、最小直接修改，以及验证命令运行前需要的生成物、初始化、迁移、fixture 或环境状态。Verification artifact 必须记录 first-principles fit、surgical change scope、fallback/redundancy review 和生成状态文件未进入 staged/tracked commit scope；delivery report 必须包含 `Cleanup`，记录 `.delivery/`、runtime settings 和 harness 创建的临时 worktree 已清理、保留或延期的证据。
