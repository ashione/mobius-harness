...
2. Select `lite` or `full` mode at the start. Do not use legacy `Lightweight`, `Standard`, or `Strict` as active mode names.
   ...
   - Treat `.delivery/`, `.delivery/hooks/`, `.claude/settings*.json`, and `.codex/settings*.json` generated by the harness as local state. They must be excluded through the target repo's `.git/info/exclude`, must not be staged or committed, and must be removed or explicitly preserved with evidence during final cleanup.
...
4. Analyze requirements first:
   ...
   - Read and record applicable repository and path-specific instructions before planning or editing, including `AGENTS.md`, `CLAUDE.md`, `GEMINI.md`, `.cursorrules`, `.cursor/rules/*`, `.github/copilot-instructions.md`, `README.md`, `CONTRIBUTING.md`, and `DEVELOPMENT.md` when present. User instructions and nearer path instructions take precedence over general harness defaults.
...
5. Build a delivery plan:
   ...
   - Record a first-principles design check: the core user or system objective, the invariant that must hold, the existing mechanism that owns that invariant, and why the selected change is the smallest direct way to satisfy it.
...
7. Implement and validate:
   - Before editing, state the exact target behavior, current behavior, owning files or functions, and why those are the minimal affected surfaces.
   - Make a surgical change: edit only the owning surface needed for the target behavior, and avoid opportunistic refactors, broad rewrites, or adjacent cleanup unless the plan records why they are necessary.
   - Do not add runtime fallbacks, compatibility shims, defensive branches, abstractions, retries, or redundant code unless a requirement, observed failure, or documented platform constraint demands them. If one is necessary, record the trigger, owner, validation, and removal or maintenance expectation.
   - Run the repository's relevant tests, type checks, builds, linters, or focused validation commands.
   - Before committing, review the diff for requirements compliance, first-principles fit, surgical change scope, fallback and redundancy, implementation quality, test adequacy, security, regressions, unsafe behavior, and unintended churn.
   - Before committing or opening a PR/MR, confirm `git diff --cached --name-only` and `git status --short` do not include harness-generated state such as `.delivery/`, `.delivery/hooks/`, `.claude/settings*.json`, or `.codex/settings*.json`. If such state is staged or tracked, unstage it or block the commit until it is removed from the commit scope.
...
9. Deliver the result:
   - Before the final report, clean up harness-created temporary worktrees and generated state files when no longer needed. Do not remove the active user workspace or any worktree with uncommitted user changes. If cleanup is unsafe or the user asked to preserve audit state, record the preserved paths, owner, reason, and ignore evidence.
   - Produce a concise delivery report covering requirements, implementation, changed files, local validation, code review, sensitive information scan, PR or MR URL, CI/CD status, cleanup, risks, and follow-ups.
...
4. `G4 Implementation`: changed files are intentional, mapped to accepted requirements, limited to the owning surface, and free of unjustified fallback or redundant code.
5. `G5 Verification`: local checks, diff review, sensitive information scan, and generated-state git hygiene are complete or explicitly marked unavailable with reason.
...
- A delivery is complete only when requirements, implementation scope, changed files, first-principles design and implementation evidence, surgical scope review, fallback/redundancy review, validation, diff review, sensitive information scan, generated-state git hygiene, PR/MR state, CI/CD state, cleanup state, residual risks, and follow-ups are all reported.
- A delivery is complete only when generated harness state is absent from staged/tracked changes and cleanup of harness-created worktrees and state files is done, not applicable, or explicitly deferred with evidence.
...
- `requirements.md`: Goal, background, success criteria, scope, non-goals, risks, open questions, user decisions, instruction evidence, and Issue and Prior Attempts.
- `plan.md`: Repo findings, prior attempt comparison, selected specialist skills, Minimum Skill Dependencies, Superpowers artifact paths or fallback, Dependency Decision, first-principles design check, implementation steps, validation strategy, Validation Prerequisites, acceptance criteria, rollback notes, and checkpoints.
- `verification.md`: Commands run, outcomes, local failures and fixes, diff review notes, first-principles fit, surgical change scope, fallback/redundancy review, sensitive information scan result, generated-state git hygiene, PR/MR URL, and CI/CD runs.
- `delivery-report.md`: Executive summary, changed files, implementation summary, validation summary, PR/MR and CI/CD status, cleanup, risks, follow-ups, release notes, and version or release report notes when applicable.
