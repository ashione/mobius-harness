...
- Mobius Harness must select `lite` or `full` mode at the start. `lite` keeps compact final-response state for low-risk work; `full` uses persisted `.delivery/runs/<run-id>/` artifacts. Legacy `Lightweight` maps to `lite`, and legacy `Standard` / `Strict` map to `full` with soft or hard hook gates.
- Mobius Harness phases must include Delegation Ledger rows so each phase records distinct subagent roles, candidate specialist skills, trigger conditions, use/not-applicable/deferred/exception/blocked/done decisions, evidence, and the handoff/return expected from delegated skills. Do not use one generic role for every phase.
...
- Mobius Harness plan and verification artifacts must enforce implementation discipline: record first-principles design evidence, identify target behavior/current behavior/owning files before edits, keep changes surgical, and reject fallback, compatibility, defensive, abstraction, retry, or redundant code unless a requirement or observed constraint demands it.
- When initializing a Mobius Harness Delivery Episode Package, use `scripts/init-delivery-run.sh`; generated mode defaults to `full`, and generated hook gates default to `[soft]` unless the user or repository policy explicitly requires `[hard]`. The script must create both the `.delivery/runs/<run-id>/` artifact package and harness-owned `.delivery/hooks/config.json`, `.delivery/hooks/agent_gate.sh`, executable `.delivery/hooks/<hook-id>.sh` gate scripts, and runtime project settings using the existing `PreToolUse` / `matcher: Bash` command-hook shape in `.claude/settings.json` or `.codex/settings.json`. Generated scaffold must stay local: add it to the target repo's `.git/info/exclude`, and use `settings.local.json` rather than modifying tracked runtime settings. Keep mode, gate strength, and runtime-specific hook wording separate: use `--mode full|lite` only to label the delivery mode, use `--gate-type soft|hard` only for blocking strength, and use `--runtime auto` by default, or pin `--runtime codex`, `--runtime claude-code`, `--runtime claude`, or `--runtime generic` when the executor must be explicit; `claude` normalizes to `claude-code` in generated artifacts.
- Mobius Harness final reports must include cleanup evidence. Before marking a delivery complete, confirm generated harness state files are not staged or tracked, remove or defer harness-created temporary worktrees and `.delivery` / runtime settings state files when safe, and record any preserved state with a reason, owner, and ignore evidence.
